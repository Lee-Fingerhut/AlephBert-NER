...
ner_path = Path("drive/MyDrive/NER/").joinpath(datetime_str)
ner_path.mkdir(parents=True, exist_ok=True)


import pandas as pd

data = pd.read_csv("morph_gold_train.csv", encoding="utf8").fillna(method="ffill")
data.tail(10)
...
tr_inputs, val_inputs, tr_tags, val_tags = train_test_split(input_ids, tags, random_state=2021, test_size=0.1)
tr_masks, val_masks, _, _ = train_test_split(attention_masks, input_ids, random_state=2021, test_size=0.1)
...
pd.to_pickle([tokenizer, tag_values], ner_path.joinpath("tokenizer_0_tags_1.pkl"))
torch.save(model, ner_path.joinpath('model.pth'))
